...
describe("Public & User Bidding API (/api/bidding-rooms)", () => {
    ...
    test("POST / should create a new bidding room for an authenticated user", async () => {
        ...
        const res = await request(app)
            ...
            .field("description", "A nice watch for testing")
        ...
        expect(res.body.room.name).toBe("Test Watch from Bidding Test");
        createdRoomId = res.body.room._id; // Save ID for subsequent tests
    });
    ...
    test("POST /:id/bids should return 403 if a user bids on their own item", async () => {
